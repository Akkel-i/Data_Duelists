...
void saveToMemoryWin(const String &identifier) {
  ...
  while (addr < EEPROM.length()) {
    ...
    if (data.identifier == identifier) {
      // Data found, increment the first int
      data.value1++;
  ...
  // If the identifier doesn't exist, create a new entry
  data.identifier = identifier;
  data.value1 = 1; // Start with 1 for the first int
  data.value2 = 0; // Initialize the second int to 0
  EEPROM.put(addr, data);
  EEPROM.commit();
}


void saveToMemoryLoss(const String &identifier) {
  int addr = 0; // Starting address in EEPROM
  int size = sizeof(Data);
  Data data;
  ...
  while (addr < EEPROM.length()) {
    ...
    if (data.identifier == identifier) {
      // Data found, increment the second int
      data.value2++;
      EEPROM.put(addr, data);
      EEPROM.commit();
      return;
  ...
  // If the identifier doesn't exist, create a new entry
  data.identifier = identifier;
  data.value1 = 0; // Initialize the first int to 0
  data.value2 = 1; // Start with 1 for the second int
  EEPROM.put(addr, data);
  EEPROM.commit();
}
